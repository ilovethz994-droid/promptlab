    ...
    singleton!.initializing = (async () => {
      try {
        ...
        if (saved !== 'system' && saved !== 'user') {
          ...
          console.log('[useBasicSubMode] First initialization complete. Persisted the default value: user')
        }
      } catch (e) {
        console.error('[useBasicSubMode] Failed to initialize. Falling back to the default value user:', e)
